...
piece_t =  3.4;   // effective stacking height per disc (mm).
                  // NOTE: the physical disc is ~8.5 mm thick, but the pieces
                  // interlock/nest when stacked so each disc only adds ~3.4 mm
                  // of height in the chute. Calibrated empirically: n_pieces=2
                  // at piece_t=8.5 produced a tube that fit 5 pieces, so
                  // piece_t was back-calculated as (tube_h - floor_t - top_clr)
                  // / n_pieces_actual = (30 - 3 - 10) / 5 = 3.4 mm.
                  // Do not change this unless you re-measure with your discs.

/* ── CHUTE PARAMETERS ────────────────────────────────────────*/

n_pieces   = 5;    // pieces per chute — controls tube height. Set to 5 for
                   // the current print. Scale up for a taller chute later.
